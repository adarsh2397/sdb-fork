...
class SiriusContext : public ClientContextState {
 ...
 private:
  ...
  // The GCS backend (async libcurl-multi reactor), owned here and borrowed by
  // the scan_manager. nullptr when no gcs_config is wired. The reactor owns its
  // own worker thread — no external thread pool required.
